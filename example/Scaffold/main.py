...
from Scaffold import Scaffold, ScaffoldClient, ScaffoldTrainer
...
strategy = Scaffold(copy.deepcopy(model_base).state_dict(), device)
w_glob_keys = model_base.state_dict().keys()
...
def inin_single_client(client_id, trainloader_idx_lst, testloader_idx_lst):
    ...
    return {
        "trainer": ScaffoldTrainer(model_, optim_, nn.CrossEntropyLoss(), device, False),
...
if __name__ == "__main__":
    ...
    for client_id in range(client_numbers):
        ...
        client_lst.append(ScaffoldClient(c_conf))
