...
class ModelFedCon(nn.Module):
    def __init__(self, base_model, out_dim, n_classes, net_configs=None):
        ...
        if (
            ...
            self.teacher_model = resnet18(True)
            self.student_model = resnet18(True)

            # self.teacher_features = nn.Sequential(*list(basemodel.children())[:-1])
            # self.teacher_features = SimpleCNN_header(
            #     input_dim=(16 * 5 * 5), hidden_dims=[120, 84], output_dim=n_classes
            # )
            # self.student_features = nn.Sequential(*list(basemodel2.children())[:-1])
            self.head = nn.Linear(512, 10)
            # self.teacher_features_proj = nn.Linear(3072, 84)
            # self.student_features = SimpleCNN_header(
            #     input_dim=(16 * 5 * 5), hidden_dims=[120, 84], output_dim=n_classes
            # )
            num_ftrs = 84
    ...
    def forward(self, x):
        ...
        th_lst = forward_backbone(self.teacher_model, x)
        sh_lst = forward_backbone(self.student_model, x)

        ty = self.head(th_lst[-1])
        sy = self.head(sh_lst[-1])

        return (th_lst, sh_lst), (ty, sy)
